...
.. slido::
    ...
    https://www.postgresql.org/docs/14/ecpg.html

    https://www.postgresql.org/docs/current/ecpg-cpp.html

    https://docs.postgresql.tw/v/14/client-interfaces/ecpg-embedded-sql-in-c
...
.. slido:: Use Another name for a Table in Query
   :class: t2c
...
.. slido::
   :class: t2c
    ...
    .. class:: rtl-h1
      ...
      نام قطعه‌ها را همراه با نام عرضه‌کنندگان‌شان بیابید
...
.. slido::
   :class: t2c
    ...
    .. class:: rtl-h1
      ...
      نام قطعاتی را بیابید که وزن‌شان دست کم از یک قطعهٔ با رنگ قرمز کمتر باشد
...
.. slido::
    ...
    .. class:: rtl-h1
        ...
        نام قطعاتی را بیابید که نام شهر آنها چهارحرفی باشد  با O آغاز شده باشد

    .. code:: sql

      select pname
      from p
      where city like 'O___'
      ;
...
.. slido::
   :class: t2c
    ...
    .. class:: rtl-h1
        ...
        نام قطعه‌هایی را بیابید که فقط عرضه کنندگان درون آن شهرها آنها را عرضه کرده باشند یا عرضه نشده باشند.
...
.. slido::
   :class: t2c
    ...
    .. class:: rtl-h1
      ...
      نام قطعه‌های متفاوتی را بیابید که عرضه‌ای از آنها وجود داشته باشد و فقط عرضه کنندگان درون آن شهرها آنها را عرضه کرده باشند
...
.. slido::
    ...
    .. image:: db/img/sql/alter_table_sqlite.png
...
.. slido::
   :id: n-supplier-red-part-qty-gt-5-id
...
.. slido:: group by
    ...
    .. class:: rtl-h2
      ...
      شمارهٔ عرضه‌کنندگان و مجموع عرضه‌های هر کدام از آنها را بیابید
    ...
    * و جمع qty های آن دسته برگردانده می‌شود.
...
.. slido::
   :class: t2c
    ...
    .. class:: rtl-h1
      ...
      شمارهٔ قطعات و مجموع عرضه‌های هر کدام از آنها را بیابید
...
.. slido::
   :id: query-inside-from-id
    ...
    .. code:: sql
      ...
      select sum(weight) as swg
      from (select distinct pn, weight
          from (p natural join sp)
            join s using(sn)
          where s.city = 'Paris'
        )
...
.. slido:: Scalar value(I)
   :id: scalar-value-1-id
    ...
    .. image:: db/img/scalar_weight_avg.png
        :class: substep
    ...
    .. image:: db/img/scalar_p_weight_min.png
        :class: substep
...
.. slido::
   :id: scalar-value-2-id
    ...
    .. image:: db/img/scalar2_sum_qty_wrong.png
        :class: substep
    ...
    .. image:: db/img/scalar2_sum_qty.png
        :class: substep

.. slido::
   :id: sum-status-in-select-id
    ...
    .. image:: db/img/sum_status_in_select.png
        :class: substep
...
.. slido::
   :id: left-outer-join1-id
    ...
    .. image:: db/img/left_outer_join1_wrong_inner_join.png
        :class: substep
    ...
    .. image:: db/img/left_outer_join1_using_scalar.png
        :class: substep
...
.. slido:: order by
   :id: order-by-asc-id
    ...
    .. image:: db/img/order_by_asc.png
        :class: center substep
...
.. slido:: order by ..... desc
    ...
    .. image:: db/img/order_by_desc.png
        :class: substep
...
.. slido:: SQLite uses the following terminology
...
.. slido:: no action, restrict, set null, set default or cascade
...
.. slido:: View
    ...
    .. image:: db/img/create_view_postgresql.png
      :width: 740px
