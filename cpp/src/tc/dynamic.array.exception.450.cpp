...
template <typename T> class ArrayCls{
 ...
 public:
  void write2file(const char*fname="a1.bin"){
    ofstream f1(fname);
    if(!f1){cout<<"cannot open file"<<endl;throw myException("File");}
    f1.write((char*)&size,sizeof(int));
    f1.write((char*)data,sizeof(T)*size);
    f1.close();
  }
  void readFromFile(const char*fname="a1.bin"){
    ifstream f1(fname);
    if(!f1){cout<<"cannot open file"<<endl;throw myException("File");}
    f1.read((char*)&size,sizeof(int));
    data=new T[size];
  ...
  ArrayCls(const ArrayCls&a){data=new T[size=a.size];
    for(int i=0;i<size;i++) data[i]=a.data[i];
  }ArrayCls(ArrayCls&&a){data=a.data;size=a.size;a.data=0;a.size=0;}
  bool search(T k)
  ...
  template<class Type> friend ostream& operator<<(ostream &o,ArrayCls<Type> a);
  };
...
int main(){
  ...
  ArrayCls<complexCls> z=f1<complexCls>(5);
  cout<<"after f1"<<endl;
  try{
  a.write2file();
  b=z;
