...
class side{
  ...
  bool operator>(const side &a){cout<<d<<" > "<<a.d<<endl; return d>a.d;}
  void show(){cout<<d;}
  friend ostream& operator<<(ostream& o1, side a1);
  friend istream& operator>>(istream& i1, side a1);
};
ostream& operator<<(ostream& o1, side a1){
  o1<<a1.d<<endl;
  return o1;
}
istream& operator>>(istream& i1, side a1){
  cin>> a1.d;
  if(a1.d<=0) throw "side must be greater than zero";
  return i1;
}
class triangle{
  side a, b, c;
