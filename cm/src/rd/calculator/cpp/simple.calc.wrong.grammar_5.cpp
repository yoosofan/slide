...
enum tokenType { ID, PLUS, MIN, DIV, MUL, END_OF,F_ERR  };
...
struct lex{char *buf=nullptr;int index=0;
  ...
  token getToken(void){token cur;cur.v=0;cur.t=F_ERR;
    ...
    else if(buf[index]=='-'){index++; cur.t=MIN;}
    else if(buf[index]=='*'){index++; cur.t=MUL;}
    else if(buf[index]=='/'){index++; cur.t=DIV;}
...
class run{lex l; token cur; public:
  ...
  double E(void){    cout<<"E" <<endl; double x=T();tokenType t1=cur.t;
    if(cur.t == PLUS || cur.t == MIN ){
      cur=l.getToken();
      if(t1==PLUS) x+= E();
      else x-=E();
    }
  ...
  double T(void){    cout<<"T" <<endl; double x=F(); tokenType t1=cur.t;
    if(cur.t == MUL || cur.t == DIV){
      cur=l.getToken();
      if(t1==MUL)
        x*=T();
      else x/=T();  
