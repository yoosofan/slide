...
#include <cstdlib>
#include <cstring>
using namespace std;enum TokenType {NUMBER, PLUS, MINUS, MUL , DIV, LPAR, RPAR, EOF};
struct Token {TokenType type; int n; double r;  string s;};
Token  t;  bool debug_mod = true; char buffer[200]; int tindex=0;
double E(void); double T(void); double F(void);void debug(const char*,unsigned,const char*s="",double d=0);void error(const char*);void read(void);void reset_all(void);
Token getToken(void){
...
double F(void){double x;debug(__func__,__LINE__);
  if(t.type==LPAR){t=getToken();x=E();if(t.type != RPAR) error(" ) is missings");}
  else if(t.type==NUMBER) x=t.n ;
  else  error("Wrong token ");
  t=getToken();debug(__func__,__LINE__,"After F :: ",x);return x;
}
int main(){while(1){reset_all();read();A();for(int i=0;i<45;i++)cout<<"=";cout<<endl;}}
void reset_all(void){tindex=0;}
void debug(const char *fname,unsigned lineno,const char *st,double val)
{ static int number_of_call = 0;number_of_call++;
...
}void error(const char* msg){debug(__func__,__LINE__,msg); exit(0);}
