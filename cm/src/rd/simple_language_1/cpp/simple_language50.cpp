...
int main(int argc, char* argv[]) {
  /*for (std::string_view arg : std::arguments())
  {std::println("Argument: {}", arg);} c++26 */
  if(argc > 1) buf = argv[1];
  else{ cout << "File name? " << endl; exit(0);}
  ifstream f1(buf); stringstream buffer;
  buffer << f1.rdbuf(); buf = buffer.str(); f1.close();
  cout << buf << endl;
  getTk(); B();
  if (tk.t != EOF_T) err();
  cout << "Accept" << endl;
